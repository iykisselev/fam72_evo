...
# Function to extract haplotypes and generate a bar plot for a given gene region
haplotype_bar <- function(vcf_file, gene, popdata) {
  # Read the VCF file
  v <- read.vcfR(vcf_file)
  
  # Convert to DNAbin and extract haplotypes with a minimum frequency of 50
  v_dnabin <- vcfR2DNAbin(v, extract.indels = FALSE)
  haps_50 <- haplotype(v_dnabin) %>% subset(minfreq = 50) %>% sort()
  
  # Pre-allocate a list to store data frames
  ...
  # Join with population data and count the number of samples per haplotype and superpopulation
  df_hist <- df %>% mutate(Sample = str_remove_all(Sample, "_.+")) %>%
    left_join(popdata[,c(2,6,7)], by = c("Sample" = "SampleID")) %>%
    group_by(Haplotype, Superpopulation) %>%
    summarise(n = n(), .groups = 'drop')
  df_hist$Haplotype <- factor(df_hist$Haplotype, levels = 
           (df_hist %>% group_by(Haplotype) %>% summarise(f = sum(n)) %>% 
              arrange(desc(f)))$Haplotype)
  
  # Plot the haplotype distribution
  plt <- ggplot(data = df_hist, aes(y = n, x = Haplotype, fill = Superpopulation)) + 
...
plots <- lapply(1:nrow(fam72_grch38), function(i) {
  ...
  haplotype_bar(vcf_filename, toupper(gene_info$V4), popdata)
})
